...
type VoiceState = "idle" | "listening" | "parsing" | "parsed" | "confirmed";
...
const VoiceGiftCreation: React.FC = () => {
  ...
  const handleToggleListening = async () => {
    if (isListening) {
      ...
      if (result && result.confidence > 0) {
        setParsedGift({
          grandchildName: result.grandchild_name || "Arjun",
          grandchildId: "22222222-2222-2222-2222-222222222222", // Seeded UUID
          riskProfile: result.risk_profile || "Balanced",
          corpus: Number(result.corpus) || 1000,
          currency: result.currency || "USD",
          releaseCondition: result.release_condition || "Graduation",
        });
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 text-center py-8">
      ...
      {/* Results Section */}
      {voiceState === "parsed" && (
        <div className="bg-card rounded-3xl border-2 border-primary/20 p-8 text-left space-y-6 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
